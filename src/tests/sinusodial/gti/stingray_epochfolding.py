...
#events = EventList().read('combined_el2.dat', 'ascii')
events = EventList().read('combined_eventlist.dat', 'ascii')
...
for p in test_periods:
    #ph, profile, profile_err = fold_events(events.time, 1/p, nbin=nbin, gti=events.gti)
...
#freq, efstat = epoch_folding_search(events.time, frequencies, nbin=nbin, gti=events.gti)
