...
events = EventList().read('eventlist.dat', 'ascii')
#print(events.time[:5])
#lc = Lightcurve.read('timeseries_0.dat', 'ascii')
#print(lc.time[:5], lc.counts[:5], lc.dt)

'''
plt.figure()
plt.plot(lc.time, lc.counts, color="blue")
plt.title("Light curve")
plt.xlabel(f"Time (s from {lc.mjdref})")
...
f_profile = 'profile_lc.png'
nbin = 10
test_periods = [10., 10.01, 10.1, 1.1]
fig, ax1 = plt.subplots()
#'''
'''
# folding using events
ph, profile, profile_err = fold_events(events.time, 1/10., nbin=nbin)
ax1 = plot_profile(ph, profile, ax=ax1)

ph, profile, profile_err = fold_events(events.time, 1/1.1, nbin=nbin)
ax1 = plot_profile(ph, profile, ax=ax1)

ph, profile, profile_err = fold_events(events.time, 1/9.66, nbin=nbin)
ax1 = plot_profile(ph, profile, ax=ax1)
'''
'''
...
frequencies = np.arange(1/(period+.1) - 200 * df, 1/(period) + 200 * df, df)

####GAUSSIAN####
# filenames for plots
#profile = 'profile.png'
f_chi2 = 'chi2_sin_lc.png'
f_chi2_fit = 'chi2_fit_sin_lc.png'
