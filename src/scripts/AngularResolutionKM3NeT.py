...
Usage: AngularResolutionKM3NeT.py -i INPUT_FILES... -o OUTPUT_DIR [--detector=<detector>] [--runtype=<runtype>] [--recotype=<recotype>] [--pltscale=<pltscale>]
...
def plot_angular_resolution_vs_energy(table, reco_type_id, reco_stage_id, reco_name, energy_range, n_bins, run_type, plt_scale, file_name):
    # Apply mask for the specified reco_type and reco_stage
    mask = (table['reco_type'] == reco_type_id) & (table['reco_stage'] == reco_stage_id)
    ...
    median_separations = np.array(mean_separations)
    ...
    output_plotname = "TestPlotAngularRes" + reco_name + "_" + run_type + "_" + plt_scale + ".png"

    plt.savefig(output_plotname)   
    plt.close()   
    print(f"Plot saved: {output_plotname}")

    hdf5_name = "AngularResolutionOverEnergy_" +  reco_name + "_" + file_name + ".hdf5"
    ...
    angular_resolution_table.write(hdf5_name, format='hdf5', overwrite=True, serialize_meta=True)
    ...
    output_histname = "HistogramSeparations_" + reco_name + "_" + run_type + "_" + plt_scale + ".png"
...
def main():
    ...
    if reco_type == 'jmuon':
        ...
        reco_stage_id = kd.reconstruction.JMUONBEGIN
    ...
    plot_angular_resolution_vs_energy(result_table, reco_type_id, reco_stage_id, reco_type , energy_range, n_bins,run_type,plt_scale,file_name)
