#!/usr/bin/env python3
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from matplotlib.colors import LogNorm
import h5py
from h5py import File
from astropy.table import Table
from astropy.coordinates import SkyCoord, AltAz
import astropy.units as u
from km3astro.frame import get_location


# ---------------------------
# Placeholder for missing imports
# ---------------------------
def load_aeff(file_path, flavor):
    with h5py.File(file_path, 'r') as f:

        aeff_1d = f[flavor]['aeff_1d'][()]
        aeff_2d = f[flavor]['aeff_2d'][()]
        e_bins = f['energy_bins'][()]
        costh_bins = f['cos_theta_bins'][()]
    return aeff_1d, aeff_2d, e_bins, costh_bins

def powerLawFlux(E, Phi0, gamma, E0=1e5):
    """
    Returns differential flux at energy E [GeV], normalized at E0 [GeV].
    Phi0 in units of GeV^-1 cm^-2 s^-1 sr^-1.
    """
    return Phi0 * (E / E0) ** (-gamma)

import numpy as np

def bounded_power_law_flux(E, Phi0, gamma, E0=1e5, E_min=None, E_max=None):
    """
    Power-law differential flux with hard cutoffs.

    Parameters
    ----------
    E : array_like
        Energy(s) in GeV.
    Phi0 : float
        Normalization at E0 (units GeV^-1 cm^-2 s^-1 sr^-1).
    gamma : float
        Spectral index (flux ~ (E/E0)^(-gamma)).
    E0 : float
        Reference energy in GeV where Phi0 is defined.
    E_min, E_max : float or None
        If provided, flux is zero for E < E_min or E > E_max.

    Returns
    -------
    flux : ndarray
        Differential flux evaluated at E.
    """
    E = np.asarray(E, dtype=float)
    flux = Phi0 * (E / E0) ** (-gamma)

    if E_min is not None:
        flux = np.where(E < E_min, 0.0, flux)
    if E_max is not None:
        flux = np.where(E > E_max, 0.0, flux)

    return flux


def phi_softbounded(E, phi0, gamma, E0, Emin, Emax, delta = 50, factor = 2, xoffset = 0):
    """
    Power-law flux with soft lower cutoff.
    
    E     : energy array
    phi0  : normalization at E0
    gamma : spectral index
    E0    : reference energy
    Emin  : characteristic lower cutoff energy
    Emax  : sharp upper cutoff
    delta : softening scale of the cutoff (controls smoothness)
    """
    # Core power-law
    flux = factor* phi0 * ((E+xoffset) / (E0-delta))**(-gamma)
    
    # Soft turn-on near Emin
    soft = 1 - np.exp(-((E+xoffset) - (Emin-delta)) / delta)
    
    # Upper cutoff
    mask =(E+xoffset) <= Emax
    return flux * soft * mask

# ---------------------------
# Plotting function
# ---------------------------
def plot_event_energies(df_events, energy_bins, total_energies, detectorname, phi0, gamma, plot_config=None, E0=1e5,Emin=7e4,Emax=6e5,source_name="Vela", delta = 50, factor = 2, xoffset = 0):
    if plot_config is None:
        plot_config = {
            "flavors": {
                "nue": {"color": "tab:blue", "label": "νe", "linewidth": 1.2},
                "anue": {"color": "tab:orange", "label": "ν̄e", "linewidth": 1.2},
                "numu": {"color": "tab:green", "label": "νμ", "linewidth": 1.2},
                "anumu": {"color": "tab:red", "label": "ν̄μ", "linewidth": 1.2},
                "nutau": {"color": "tab:purple", "label": "ντ", "linewidth": 1.2},
                "anutau": {"color": "tab:brown", "label": "ν̄τ", "linewidth": 1.2},
            },
            "shower_fill": {"color": "skyblue", "alpha": 0.4, "label": "Total showers"},
            "track_fill": {"color": "salmon", "alpha": 0.4, "label": "Total tracks"},
            "total_line": {"color": "black", "linewidth": 2.0, "label": "Total flux"}
        }

    energy_vals = 0.5 * (energy_bins[:-1] + energy_bins[1:])

    fig, ax1 = plt.subplots(figsize=(7, 4))

    def bin_counts(energies):
        return np.histogram(energies, bins=energy_bins)[0]

    shower_energies = df_events[df_events["event_type"] == "shower"]["energy"]
    track_energies = df_events[df_events["event_type"] == "track"]["energy"]

    n_showers = len(shower_energies)
    n_tracks = len(track_energies)

    shower_counts = bin_counts(shower_energies)
    track_counts = bin_counts(track_energies)

    ax1.fill_between(
        energy_vals, track_counts, step='mid',
        color=plot_config["track_fill"]["color"],
        alpha=plot_config["track_fill"]["alpha"],
        label=f"{plot_config['track_fill']['label']} ({n_tracks} events)"
    )

    ax1.fill_between(
        energy_vals, shower_counts, step='mid',
        color=plot_config["shower_fill"]["color"],
        alpha=plot_config["shower_fill"]["alpha"],
        label=f"{plot_config['shower_fill']['label']} ({n_showers} events)"
    )

    for flavor in df_events["flavor"].unique():
        energies = df_events[df_events["flavor"] == flavor]["energy"]
        n_events_flavor = len(energies)
        style = plot_config["flavors"].get(flavor, {})
        ax1.hist(
            energies, bins=energy_bins, histtype='step',
            linewidth=style.get("linewidth", 1.0),
            color=style.get("color", "gray"),
            label=f"{style.get('label', flavor)} ({n_events_flavor} events)",
            linestyle=style.get("linestyle", "-")
        )

    n_total = len(total_energies)
    ax1.hist(
        total_energies, bins=energy_bins, histtype='step',
        linewidth=plot_config["total_line"]["linewidth"],
        color=plot_config["total_line"]["color"],
        label=f"{plot_config['total_line']['label']} ({n_total} events)",
        linestyle=plot_config["total_line"].get("linestyle", "-")
    )

    # --- New: Add secondary y-axis for flux ---
    ax2 = ax1.twinx()
    #flux_vals = powerLawFlux(energy_vals, phi0, gamma, E0)  # in physical units
    flux_vals = bounded_power_law_flux(energy_vals, phi0, gamma, E0=E0, E_min=Emin, E_max = Emax)
    #flux_vals = phi_softbounded(energy_vals, phi0, gamma, E0, Emin, Emax, delta, factor, xoffset)
    #flux_vals = bounded_power_law_flux(energy_vals,phi0,gamma,E_min=5e4,E_max=5e5)
    ax2.plot(
        energy_vals, flux_vals,
        color='purple', alpha=0.8, linestyle='--', linewidth=2,
        label=f"Input PowerLaw Flux (γ={gamma})"
    )
    #color='forestgreen'
    ax2.set_ylabel(r"Flux [GeV$^{-1}$ m$^{-2}$ s$^{-1}$]")
    ax2.set_yscale('log')
    ax2.set_ylim([1e-18,1])

    # --- Formatting ---
    ax1.set_xscale('log')
    ax1.set_yscale('log')
    ax1.set_xlabel("Energy [GeV]")
    ax1.set_ylabel("Number of events")
    ax1.set_title(f"Simulated Detected Events & Input Flux ({detectorname}, {phi0:.2e}, {gamma}, {source_name})")
    ax1.grid(True, which='both', ls='--', alpha=0.5)

    # Combine legends from both axes
    lines_1, labels_1 = ax1.get_legend_handles_labels()
    lines_2, labels_2 = ax2.get_legend_handles_labels()
    #legend = ax1.legend(lines_1 + lines_2, labels_1 + labels_2, framealpha=0.9, loc='best', fontsize='small')
    legend = fig.legend(
        lines_1 + lines_2, labels_1 + labels_2,
        loc="upper left",
        bbox_to_anchor=(0.02, 0.98),   # relative to axes area
        bbox_transform=ax1.transAxes,  # <-- use axes fraction coords
        fontsize="small", framealpha=0.9
        )
    plt.tight_layout()
    plt.savefig(f"detected_events/2025-08-23/detected_energies_with_fluxaxis_phi0{phi0}_{detectorname}_gamma{gamma}_total{n_total}_E0{E0}_bounded150d_{source_name}.png")
    plt.close(fig)




# ---------------------------
# Simulation helpers
# ---------------------------
def simulate_events_with_2d_aeff(phi0, energy_bins, aeff_2d, theta_bin_edges,
                                 source_coord, times, location,
                                 gamma=1.5, T=1e6, Omega=2*np.pi, n_events=None,
                                 E0=1e5, plot=False, Emin=7e4, Emax=6e5, delta = 50, factor = 2, xoffset = 0):
    from astropy.time import Time

    E_centers = 0.5 * (energy_bins[:-1] + energy_bins[1:])
    dE = np.diff(energy_bins)

    flux = powerLawFlux(E_centers, phi0, gamma, E0)
    #flux = bounded_power_law_flux(E_centers, phi0, gamma, E0=E0, E_min=Emin, E_max = Emax)
    #flux = phi_softbounded(E_centers, phi0, gamma, E0, Emin, Emax, delta, factor, xoffset)
    altaz_frame = AltAz(obstime=Time(times), location=location)
    source_altaz = source_coord.transform_to(altaz_frame)
    theta_vals = (90 * u.deg - source_altaz.alt).to(u.rad).value

    weights = np.zeros_like(E_centers)
    dt = T / len(theta_vals)

    for theta in theta_vals:
        theta_idx = np.digitize(theta, theta_bin_edges) - 1
        theta_idx = np.clip(theta_idx, 0, aeff_2d.shape[1] - 1)
        aeff_theta = aeff_2d[:, theta_idx]
        weights += flux * aeff_theta * dE * dt * Omega

    MAX_EVENTS = int(1e7)
    if n_events is None:
        expected_total = np.sum(weights)
        n_events = np.random.poisson(expected_total)
        if n_events > MAX_EVENTS:
            raise RuntimeError(
            f"Too many events expected ({n_events:.2e} > {MAX_EVENTS:.2e}). "
            "Skipping simulation.")
            #print(f"Warning: n_events={n_events} is very large; truncating to {MAX_EVENTS}.")
            #n_events = MAX_EVENTS


    if n_events == 0:
        if plot:
            print("No events to simulate (n_events=0).")
        return np.array([]), 0

    p_bins = weights / np.sum(weights)
    sampled_bins = np.random.choice(len(p_bins), size=n_events, p=p_bins)

    log_E_low = np.log10(energy_bins[sampled_bins])
    log_E_high = np.log10(energy_bins[sampled_bins + 1])
    simulated_logE = np.random.uniform(log_E_low, log_E_high)
    simulated_energies = 10**simulated_logE

    if plot:
        plt.figure(figsize=(8, 5))
        plt.hist(simulated_energies, bins=energy_bins, histtype='step', color='blue', lw=1.5)
        plt.xscale('log')
        plt.yscale('log')
        plt.xlabel("Energy [GeV]")
        plt.ylabel("Number of events")
        plt.title(f"Simulated Detected Event Energies\nPhi0={phi0:.1e}, gamma={gamma}")
        plt.grid(True, which='both', ls='--', alpha=0.5)
        plt.tight_layout()
        plt.show()

    return simulated_energies, n_events


def simulate_events_all_flavors(phi0, energy_bins, theta_bin_edges, aeff_dict,
                                source_coord, times, location, detectorname,
                                gamma=1.5, days=90, Omega=2*np.pi,
                                n_events=None, E0=1e5, plot=False, plot_config=None,Emin=None,Emax=None, delta = 50, factor = 2, xoffset = 0):
    total_events = 0
    total_energies = []
    event_records = []
    shower_energies = []
    track_energies = []

    T = days * 86400

    FLAVOR_EVENTTYPE_MAP = {
        "nue": "shower", "anue": "shower",
        "numu": "track", "anumu": "track",
        "nutau": "shower", "anutau": "shower"
    }

    n_flavors = len(aeff_dict)
    phi0_per_flavor = phi0 / n_flavors

    for flavor, aeff_2d in aeff_dict.items():
        simulated_energies, n_evt = simulate_events_with_2d_aeff(
            phi0=phi0_per_flavor, energy_bins=energy_bins,
            aeff_2d=aeff_2d, theta_bin_edges=theta_bin_edges,
            source_coord=source_coord, times=times, location=location,
            gamma=gamma, T=T, Omega=Omega, E0=E0, plot=False, Emin=Emin, Emax=Emax
        )

        total_events += n_evt
        total_energies.extend(simulated_energies)

        event_type = FLAVOR_EVENTTYPE_MAP.get(flavor, "shower")
        if event_type == "shower":
            shower_energies.extend(simulated_energies)
        elif event_type == "track":
            track_energies.extend(simulated_energies)

        records = [{"energy": energy, "flavor": flavor, "event_type": event_type}
                   for energy in simulated_energies]
        event_records.extend(records)

    df_events = pd.DataFrame(event_records)
    #em_shower_events = df_events[df_events["flavor"].isin(["nue", "anue"])].shape[0]

    #print("em_shower_events:", em_shower_events)
    print("df_events:", df_events)

    if plot and len(total_energies) > 0:
        plot_event_energies(df_events, energy_bins, total_energies,
                            detectorname, phi0, gamma, plot_config)

    return (np.array(total_energies), total_events,
            df_events, np.array(shower_energies), np.array(track_energies))


def run_simulation_for_detectors(detectorname, phi0_list, gamma_list,
                                 source_file, Eminlist=None, Emaxlist=None, plot_config=None, E0=1e5,Emin=7e4,Emax=6e5,source_name="Vela", delta = 50, factor = 2, xoffset = 0):
    if detectorname.lower() == "orca":
        aeff_file = "aeff_summary_v9_flavors_orca.hdf5"
        input_file = "/home/wecapstor3/capn/capn107h/combined_eventlists/orca_KM3NeT_00000148_0001_combined_1008510events_mc.hdf5"
        input_file = "/home/hpc/capn/capn107h/software/nextflow_output/combined_eventlists/orca_KM3NeT_00000148_0001_combined_1787352events.hdf5"
    elif detectorname.lower() == "arca":
        aeff_file = "aeff_summary_v9_flavors_arca.hdf5"
        input_file = "/home/wecapstor3/capn/capn107h/combined_eventlists/arca_KM3NeT_00000133_0001_combined_30929077events_data.hdf5"
        input_file = "/home/hpc/capn/capn107h/software/nextflow_output/combined_eventlists/arca_KM3NeT_00000133_0001_combined_477917events.hdf5"
    else:
        raise ValueError(f"Detector name '{detectorname}' not recognized.")

    aeff_dict = {}
    for flavor in ['nue', 'anue', 'numu', 'anumu', 'nutau', 'anutau']:
        _, aeff_2d, e_bins, cos_theta_bins = load_aeff(aeff_file, flavor)
        aeff_dict[flavor] = aeff_2d

    with h5py.File(source_file, 'r') as f:
        ra = f['skycoord/ra'][()]
        dec = f['skycoord/dec'][()]
        source_skycoord = SkyCoord(ra=ra * u.deg, dec=dec * u.deg)

    with File(input_file, 'r') as h5_file:
        eventList = Table.read(h5_file)
        time = eventList['time']

    detector_location = get_location(detectorname)
    observation_times = np.linspace(time.min(), time.max(), 1000)

    if Eminlist is not None:
        if Emaxlist is not None:
            print("Specifying...")

            

    energy_bins = e_bins
    theta_bin_edges = np.arccos(cos_theta_bins[::-1])
    results = {}
    for phi0 in phi0_list:
        for gamma in gamma_list:
            for Emin in (Eminlist if Eminlist is not None else [Emin]):
                for Emax in (Emaxlist if Emaxlist is not None else [Emax]):
                    try:
                        energies, n_total, event_df, _, _ = simulate_events_all_flavors(
                            phi0=phi0, energy_bins=energy_bins, theta_bin_edges=theta_bin_edges,
                            aeff_dict=aeff_dict, source_coord=source_skycoord, times=observation_times,
                            location=detector_location, detectorname=detectorname, days=150,
                            gamma=gamma, plot=False, plot_config=plot_config, E0=E0,
                            Emin=Emin, Emax=Emax, delta=delta, factor=factor, xoffset=xoffset)
                            
                        results[(phi0, gamma, Emin, Emax)] = (event_df, energies)
                        print(f"[{detectorname}] φ0={phi0}, γ={gamma}, Emin={Emin}, Emax={Emax} → {n_total} events")

                        plot_event_energies(event_df, energy_bins, energies, detectorname, phi0, gamma, plot_config, E0=E0,Emin=Emin,Emax=Emax,source_name=source_name, delta = delta, factor = factor, xoffset = xoffset)
           
                    except Exception as e:
                        print(f"    Failed for {detectorname}, φ0={phi0}, γ={gamma}, Emin={Emin}, Emax={Emax} : {e}")

    #if results:
    #    plot_multiple_combinations(results, energy_bins, detectorname, E0=E0, Emin=Emin, Emax=Emax, source_name=source_name, delta = delta, factor = factor, xoffset = xoffset)

def plot_multiple_combinations(results_dict, energy_bins, detectorname, E0=1e5, Emin=7e4, Emax=6e5, source_name = "Vela", delta = 50, factor = 2, xoffset = 0):
    """
    Plot histograms of detected events + flux curves for multiple (phi0, gamma) combinations.

    Parameters
    ----------
    results_dict : dict
        Keys: (phi0, gamma), Values: (df_events, total_energies).
    energy_bins : array
        Energy bin edges.
    detectorname : str
        Name of detector.
    E0 : float
        Reference energy for power-law flux normalization.
    """
    energy_vals = 0.5 * (energy_bins[:-1] + energy_bins[1:])
    fig, ax1 = plt.subplots(figsize=(8, 5))

    # Primary y-axis → detected events
    ax1.set_xscale("log")
    ax1.set_yscale("log")
    ax1.set_xlabel("Energy [GeV]")
    ax1.set_ylabel("Number of events")

    # Secondary y-axis → flux
    ax2 = ax1.twinx()
    ax2.set_yscale("log")
    ax2.set_ylabel(r"Flux [GeV$^{-1}$ m$^{-2}$ s$^{-1}$]")
    ax2.set_ylim([1e-20,1])

    # Cycle through combinations
    colors = plt.cm.tab10(np.linspace(0, 1, len(results_dict)))  # distinct colors

    colors = ['teal','steelblue','darkblue','firebrick','darkorange'] 
    alphas = [1,0.9,0.8,0.7,0.6]


    for (i, ((phi0, gamma, Emin, Emax), (df_events, total_energies))) in enumerate(results_dict.items()):
        color = colors[i % len(colors)]
        n_total = len(total_energies)

        ax1.hist(total_energies, bins=energy_bins, histtype="step",
                linewidth=1.8, color=color,
                label=f"Events φ₀={phi0:.1e}, γ={gamma}, Emin={Emin:.1e}, Emax={Emax:.1e} ({n_total})")

        energy_vals_flux = np.logspace(4, 7, 300)
        flux_vals = bounded_power_law_flux(energy_vals_flux, phi0, gamma, E0, Emin, Emax)
        ax2.plot(energy_vals_flux, flux_vals, linestyle="--", linewidth=1.5,
                color=color, alpha=0.8,
                label=f"Flux φ₀={phi0:.1e}, γ={gamma}, Emin={Emin:.1e}, Emax={Emax:.1e}")

    # Title + grid
    ax1.set_title(fr"Detected Events & Flux for varying $\Phi_0$ at $\gamma=2.0$ (Source: {source_name})")
    ax1.grid(True, which="both", ls="--", alpha=0.4)

    # Merge legends
    lines1, labels1 = ax1.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    #ax1.legend(lines1 + lines2, labels1 + labels2,framealpha=0.9, fontsize="small", loc="lower left")
    legend = fig.legend(
        lines1 + lines2, labels1 + labels2,
        loc="upper left",
        bbox_to_anchor=(0.02, 0.98),   # relative to axes area
        bbox_transform=ax1.transAxes,  # <-- use axes fraction coords
        fontsize="small", framealpha=0.9
        )
    plt.tight_layout()
    plt.savefig(f"detected_events/2025-08-23/detected_energies_with_fluxaxis_comparisonphi0{detectorname}_unbounded150d_{source_name}.png")
    plt.close(fig)

def expected_events_heatmap(detectorname, phi0_values, gamma_values, source_file,
                            E0=1e5, days=150, Omega=2*np.pi, cmap="viridis",n_runs=5,Emin=7e4, Emax=6e5,source_name="Vela"):
    """
    Compute expected number of detected events for all (phi0, gamma) combinations
    and plot as a heatmap with logarithmic color scale.
    """

    # --- Load detector-specific inputs ---
    if detectorname.lower() == "orca":
        aeff_file = "aeff_summary_v9_flavors_orca.hdf5"
        input_file = "/home/hpc/capn/capn107h/software/nextflow_output/combined_eventlists/orca_KM3NeT_00000148_0001_combined_1787352events.hdf5"
    elif detectorname.lower() == "arca":
        aeff_file = "aeff_summary_v9_flavors_arca.hdf5"
        input_file = "/home/hpc/capn/capn107h/software/nextflow_output/combined_eventlists/arca_KM3NeT_00000133_0001_combined_477917events.hdf5"
    else:
        raise ValueError(f"Detector name '{detectorname}' not recognized.")

    # --- Load Aeff tables ---
    aeff_dict = {}
    for flavor in ['nue','anue','numu','anumu','nutau','anutau']:
        _, aeff_2d, e_bins, cos_theta_bins = load_aeff(aeff_file, flavor)
        aeff_dict[flavor] = aeff_2d

    # --- Source coordinates ---
    with h5py.File(source_file, 'r') as f:
        ra = f['skycoord/ra'][()]
        dec = f['skycoord/dec'][()]
        source_skycoord = SkyCoord(ra=ra * u.deg, dec=dec * u.deg)

    # --- Observation times (reuse MC input file for time span) ---
    with File(input_file, 'r') as h5_file:
        eventList = Table.read(h5_file)
        time = eventList['time']
    detector_location = get_location(detectorname)
    observation_times = np.linspace(time.min(), time.max(), 200)

    energy_bins = e_bins
    theta_bin_edges = np.arccos(cos_theta_bins[::-1])
    T = days * 86400

    # --- Allocate results matrix ---
    heatmap_data = np.zeros((len(gamma_values), len(phi0_values)))

    # --- Loop over parameter space ---
    for ig, gamma in enumerate(gamma_values):
        for ip, phi0 in enumerate(phi0_values):
            totals = []
            for run in range(n_runs):
                try:
                    _, n_total, _, _, _ = simulate_events_all_flavors(
                        phi0=phi0, energy_bins=energy_bins, theta_bin_edges=theta_bin_edges,
                        aeff_dict=aeff_dict, source_coord=source_skycoord,
                        times=observation_times, location=detector_location,
                        detectorname=detectorname, days=days, gamma=gamma,
                        Omega=Omega, E0=E0, plot=False, Emin=Emin, Emax=Emax
                    ) 
                    totals.append(n_total)
                except Exception as e:
                    print(f"    Failed run for φ0={phi0}, γ={gamma}: {e}")
            if totals:
                mean_total = np.mean(totals)
                if mean_total < 1: mean_total = 0
                heatmap_data[ig, ip] = mean_total
                print(f"[{detectorname}] φ0={phi0:.2e}, γ={gamma:.2f} → "
                      f"mean={mean_total:.1f} (over {len(totals)} runs)")
            else:
                heatmap_data[ig, ip] = 0



    # Example heatmap_datas
    # Replace this with your actual data array
    # For demonstration, let's assume it's shape (len(phi0_values), len(gamma_values))
      # fine linear gamma

    # Simulated heatmap data
    #heatmap_data = np.random.rand(len(phi0_values), len(gamma_values)) 

    fig, ax = plt.subplots(figsize=(8, 6))

    heatmap_data_plot = heatmap_data.T  # transpose so y=phi0, x=gamma

    gamma_edges = np.linspace(min(gamma_values), max(gamma_values), len(gamma_values)+1)
    phi0_edges = np.geomspace(min(phi0_values), max(phi0_values), len(phi0_values)+1)

    mesh = ax.pcolormesh(
        gamma_edges, phi0_edges, heatmap_data_plot,
        #norm=LogNorm(vmin=min(heatmap_data_plot[heatmap_data_plot>0]),
        #            vmax=np.nanmax(heatmap_data_plot)),
        norm=LogNorm(vmin=1, vmax=2e7),
        cmap='viridis'
    )
    ax.set_yscale("log")

    ax.set_xlabel(r"Spectral Index $\gamma$")
    ax.set_ylabel(r"Flux Normalization $\Phi_0$ [GeV$^{-1}$ m$^{-2}$ s$^{-1}$]")
    ax.set_title(fr"Expected Events with $E_0={E0:.0f}$ GeV in {days:.1f} days (Source: {source_name})")

    cbar = fig.colorbar(mesh, ax=ax)
    cbar.set_label("Expected number of events")

    levels = [10, 100, 1000]  # desired contour levels
    # Use the same grid for contouring (must match mesh shape)
    gamma_grid, phi0_grid = np.meshgrid(gamma_values, phi0_values)
    contours = ax.contour(gamma_grid, phi0_grid, heatmap_data_plot,
                        levels=levels, colors='white', linewidths=1.5)
    ax.clabel(contours, fmt='%d', colors='white', fontsize=10)

    plt.tight_layout()
    plt.savefig(f"detected_events/2025-08-19/heatmap_expected_events_{source_name}_{str(E0)}bounded_{detectorname}.png")
    plt.close(fig)

    return heatmap_data

def expected_events_heatmap_combined(phi0_values, gamma_values, source_file,
                                     E0=1e5, days=150, Omega=2*np.pi,
                                     cmap="viridis", n_runs=5,
                                     Emin=7e4, Emax=6e5, source_name="Vela"):
    """
    Compute combined expected number of detected events for ORCA + ARCA
    and plot as a heatmap. Only keeps cells where both detectors have valid data.
    """

    # Run for both detectors
    heatmap_orca = expected_events_heatmap("orca", phi0_values, gamma_values,
                                           source_file, E0=E0, days=days,
                                           Omega=Omega, cmap=cmap, n_runs=n_runs,
                                           Emin=Emin, Emax=Emax, source_name=source_name)

    heatmap_arca = expected_events_heatmap("arca", phi0_values, gamma_values,
                                           source_file, E0=E0, days=days,
                                           Omega=Omega, cmap=cmap, n_runs=n_runs,
                                           Emin=Emin, Emax=Emax, source_name=source_name)

    # Combined result – only valid where both are > 0
    combined = np.where((heatmap_arca > 0),
                        heatmap_orca + heatmap_arca, 0)

    vmax = np.nanmax(combined)
    if vmax <= 0 or np.isnan(vmax):
        print("⚠️ No valid combined events found for ORCA + ARCA – skipping plot.")
        return combined

    # Plot combined heatmap
    fig, ax = plt.subplots(figsize=(8, 6))

    combined_plot = combined.T  # transpose so y=phi0, x=gamma

    gamma_edges = np.linspace(min(gamma_values), max(gamma_values), len(gamma_values)+1)
    phi0_edges = np.geomspace(min(phi0_values), max(phi0_values), len(phi0_values)+1)

    mesh = ax.pcolormesh(
        gamma_edges, phi0_edges, combined_plot,
        norm=LogNorm(vmin=1, vmax=vmax),
        cmap=cmap
    )
    ax.set_yscale("log")
    ax.set_xlabel(r"Spectral Index $\gamma$")
    ax.set_ylabel(r"Flux Normalization $\Phi_0$ [GeV$^{-1}$ m$^{-2}$ s$^{-1}$]")
    ax.set_title(fr"Combined Expected Events $E_0={E0:.0f}$ GeV in {days:.1f} days (Source: {source_name})")

    cbar = fig.colorbar(mesh, ax=ax)
    cbar.set_label("Expected number of events (combined)")

    # Optional: contour lines
    levels = [10, 100, 1000]
    gamma_grid, phi0_grid = np.meshgrid(gamma_values, phi0_values)
    contours = ax.contour(gamma_grid, phi0_grid, combined_plot,
                          levels=levels, colors='white', linewidths=1.5)
    ax.clabel(contours, fmt='%d', colors='white', fontsize=10)

    plt.tight_layout()
    plt.savefig(f"detected_events/2025-08-23/heatmap_expected_events_combined_{source_name}_{str(E0)}bounded.png")
    plt.close(fig)

    return combined



# ---------------------------
# Main script entry
# ---------------------------
if __name__ == "__main__":
    plot_config = {
        "flavors": {
            "nue":    {"color": "darkblue", "label": r"$\nu_e$", "linestyle": "-", "linewidth": 1.5},
            "anue":   {"color": "darkblue", "label": r"$\bar\nu_e$", "linestyle": "dotted", "linewidth": 2},
            "numu":   {"color": "firebrick", "label": r"$\nu_\mu$", "linestyle": "-", "linewidth": 1.5},
            "anumu":  {"color": "firebrick", "label": r"$\bar\nu_\mu$", "linestyle": "dotted", "linewidth": 2},
            "nutau":  {"color": "gold", "label": r"$\nu_\tau$", "linestyle": "-", "linewidth": 1.5},
            "anutau": {"color": "gold", "label": r"$\bar\nu_\tau$", "linestyle": "dotted", "linewidth": 2},
        },
        "shower_fill": {"color": "skyblue", "alpha": 0.8, "label": "Total showers"},
        "track_fill": {"color": "salmon", "alpha": 0.4, "label": "Total tracks"},
        "total_line": {"color": "black", "linewidth": 2, "label": "Total flux", "linestyle": "-"}
    }


    params1_vela = {
        "E": np.logspace(4, 8, 500),
        "E0": 1.95e5,
        "phi0": 8e-15,
        "gamma": 2.1,
        "Emin": 2e5,
        "Emax": 2e6,
        'delta': 30000,
        'xoffset':15000 
    }
    params1_crab = {
        "E": np.logspace(4, 8, 500),
        "E0": 6.9e4,
        "phi0": 9e-12, #
        "gamma": 2.1,
        "Emin": 7e4,
        "Emax": 6e5,
        'delta': 10000,
        'xoffset':0 
    }


    factor=2
    #phi0_values = [ 1e-10, 1e-11, 1e-12]
    #phi0_values = [1e-11]
    phi0_values = [1e-13]
    #gamma_values = [1.5,1.75,2.0,2.25,2.5]
    #phi0_values = [1,1e-1,1e-2,1e-3,1e-4]
    gamma_values = [2]
    #source_file = "/home/hpc/capn/capn107h/software/psr/workflows/km3net/km3net_analysis/inputs/sources/Vela_X-1.h5"
    source_file = "/home/hpc/capn/capn107h/software/psr/workflows/km3net/km3net_analysis/inputs/sources/CrabPulsar.h5"
    source_name = "Crab"
    Emin = 7e4
    Emax = 6e5
    delta = 10000
    xoffset = 0
    #E0 = 6.9e4
    E0 = Emin
    phi0_values = [1e-13]
    gamma_values = [2]
    #run_simulation_for_detectors("arca", phi0_values, gamma_values,source_file=source_file,plot_config=plot_config, E0=E0,Emin=Emin,Emax=Emax,source_name=source_name,factor = factor, xoffset = xoffset)
    
    Emin = 5e4
    Emax = 6e5
    delta = 10000
    xoffset = 0
    #E0 = 6.9e4
    E0 = Emin
    phi0_values = [7e-14]
    gamma_values = [2.5]
    #run_simulation_for_detectors("arca", phi0_values, gamma_values,source_file=source_file,plot_config=plot_config, E0=E0,Emin=Emin,Emax=Emax,source_name=source_name,factor = factor, xoffset = xoffset)
    
    source_file = "/home/hpc/capn/capn107h/software/psr/workflows/km3net/km3net_analysis/inputs/sources/Vela_X-1.h5"
    source_name = "Vela"
    Emin = 2e5
    Emax = 2e6
    delta = 30000
    xoffset = 15000
    phi0_values = [8e-15]
    gamma_values = [2.0]
    #E0=1.95e5
    E0=Emin
    #phi0_values = [1e-10*0.1225,(1e-12)*0.1225]
    #run_simulation_for_detectors("orca", phi0_values, gamma_values,source_file=source_file, plot_config=plot_config)
    
    #run_simulation_for_detectors("arca", phi0_values, gamma_values,source_file=source_file,plot_config=plot_config, E0=E0,Emin=Emin,Emax=Emax,source_name=source_name,factor = factor, xoffset = xoffset)
    
    Emin = 1.3e5
    Emax = 2e6
    delta = 30000
    xoffset = 15000
    phi0_values = [5e-15]
    #E0=1.95e5
    E0=Emin
    gamma_values = [2.5]
    #phi0_values = [1e-10*0.1225,(1e-12)*0.1225]
    #run_simulation_for_detectors("orca", phi0_values, gamma_values,source_file=source_file, plot_config=plot_config)
    
    #run_simulation_for_detectors("arca", phi0_values, gamma_values,source_file=source_file,plot_config=plot_config, E0=E0,Emin=Emin,Emax=Emax,source_name=source_name,factor = factor, xoffset = xoffset)


    #phi0_values = [1, 1e-1, 1e-2, 1e-3, 1e-4]
    #gamma_values = [1.5, 1.75, 2.0, 2.25, 2.5]
    #source_file = "/home/hpc/capn/capn107h/software/psr/workflows/km3net/km3net_analysis/inputs/sources/Vela_X-1.h5"
    #phi0_values = np.logspace(-14, -6, 25)  # fine log-spaced phi0
    #gamma_values = np.linspace(1.5, 3, 25)
    #expected_events_heatmap("arca", phi0_values, gamma_values, source_file=source_file, E0=E0, Emin=Emin, Emax=Emax, source_name=source_name)


    #phi0_values = np.logspace(-14, -6, 25)
    #gamma_values = np.linspace(1.5, 3, 25)
    phi0_values = np.logspace(-4, 2, 25)
    gamma_values = np.linspace(1.5, 3, 25)
    E0 = 1
    combined = expected_events_heatmap_combined(phi0_values, gamma_values, source_file,
                                            E0=E0, Emin=Emin, Emax=Emax, source_name=source_name)
