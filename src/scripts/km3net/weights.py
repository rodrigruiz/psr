...
# oscillated weight
def get_weight(is_cc, pdg, weight_one_year, energy, dir_z, osc):
    if is_cc:
        return get_weight_cc(pdg, weight_one_year, energy, dir_z, osc)
    else:
        return get_weight_nc(pdg, weight_one_year, energy, dir_z)


def get_weight_cc(pdg, weight_one_year, energy, dir_z, osc):
    if pdg == 12:
        w_osc = _get_weight_nue(energy, dir_z, osc)
    elif pdg == -12:
        w_osc = _get_weight_anue(energy, dir_z, osc)
    elif pdg == 14:
        w_osc = _get_weight_numu(energy, dir_z, osc)
    elif pdg == -14:
        w_osc = _get_weight_anumu(energy, dir_z, osc)
    elif pdg == 16:
        w_osc = _get_weight_nutau(energy, dir_z, osc)
    elif pdg == -16:
        w_osc = _get_weight_anutau(energy, dir_z, osc)
    else:
        w_osc = np.nan
...
def _get_weight_nue(energy, dir_z, osc):
    ...
    if osc is True:
        ...
        w_osc = flux_numu + flux_nue

    return w_osc


def _get_weight_anue(energy, dir_z, osc):
    ...
    if osc is True:
        ...
        w_osc = flux_anumu + flux_anue

    return w_osc


def _get_weight_numu(energy, dir_z, osc):
    flux_numu = get_honda_flux('numu', energy, dir_z)
    flux_nue = get_honda_flux('nue', energy, dir_z)
...
def _get_weight_anumu(energy, dir_z, osc):
    flux_anumu = get_honda_flux('anumu', energy, dir_z)
    flux_anue = get_honda_flux('anue', energy, dir_z)
...
def _get_weight_nutau(energy, dir_z, osc):
    flux_numu = get_honda_flux('numu', energy, dir_z)
    flux_nue = get_honda_flux('nue', energy, dir_z)
...
def _get_weight_anutau(energy, dir_z, osc):
    flux_anumu = get_honda_flux('anumu', energy, dir_z)
    flux_anue = get_honda_flux('anue', energy, dir_z)
...
def get_weights_pid_output(df):
    ...
    for idx in indices:
        ...
        w_one = w_one_sec[idx]
        w = get_weight(is_cc, pdg_id, w_one, e, z, osc)  # oscillated weight [1/s]
        w_osc[idx] = w  # [0, :]
