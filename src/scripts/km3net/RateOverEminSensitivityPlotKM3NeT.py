...
def main():
    ...
    if not common_prefix:
        common_prefix = "Test"
    output_file = os.path.join(output_dir, f"{common_prefix}_SensitvityRateOverE_min.hdf5")
    output_plot = os.path.join(output_dir, f"{common_prefix}_SensitvityRateOverE_min_plot.png")
    ...
    # df.write(output_file, format='hdf5', path='histogram_data', overwrite=True, serialize_meta = True)
    ...
    c = plt.pcolormesh(gammas, snrs, pval_array, shading='nearest', cmap='viridis', norm=LogNorm())

    # Sort values by gamma for correct line plot
    gamma_sorted, r5sigma_sorted = zip(*sorted(zip(gamma_list, r5sigma_list)))
    ...
    plt.xlabel(r'$E_{min} of Energy Spectrum$')
    plt.ylabel(r'Signal Rate [1/d]')
    plt.xscale('log')
    #plt.ylim([7,15])
    plt.title('p-value over signal rate and E_min')
    plt.legend()
    plt.tight_layout()
