...
workflow{
    ...
    def ratiolist = [0.1,0.15,0.2,0.25,0.3,0.35,0.4] //(input.ratio_min..input.ratio_max).step(input.ratio_step).toList()
    ...
    combinedChannel = ratios.combine(files).flatMap { ratio, file -> tuple(ratio, file) }

    // Split the combined channel into two separate channels
...
workflow {
    def ratiolist = [0.1, 0.2, 0.3, 0.4]  // example ratio values
    ...
    combinedChannel = ratios.combine(files)
    .groupTuple(by:0)
    .view()

    convert = ConvertFilesKM3NeT(combinedChannel)
    event_list = CreateEventListKM3NeT(convert.out, input.source_file, input.dist, input.energy_threshold)
    corrected_list = CorrectEventListKM3NeT(event_list.out, input.source_file)
    injected_signal = InjectSignalKM3NeT(corrected_list.out, input.frequency, combinedChannel, input.pulseshape, input.df, input.baseline, input.a, input.phi, input.kappa)
    combined_list = CombineEventListsKM3NeT(injected_signal.out.collect())
    epoch_folding = EpochFoldingKM3NeT(combined_list.out, input.frequency, input.number_of_testf, input.testf_df, input.nbin)

    // runForRatio(combinedChannel)
    //.view()
